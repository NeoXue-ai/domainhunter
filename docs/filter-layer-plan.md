...
> 记录于 2026-09-01。采集层之后的过滤漏斗：把 CT log 采集输出（first-seen 注册级域名）过滤成"真新域名"候选队列。
> 状态：**S1-S5 全部实现**（2026-09-01），端到端验证完成。

## 目标

把"新签发的证书域名"过滤成"真新域名"，最终进人类 review 队列。
...
采集层输出：first-seen 注册级域名（估每天几百~几千）
  ...
  ≤30 天 Tier 1 / 31-90 天 Tier 2 / >90 天丢弃
...
- **阈值可配置**（`--min-age-days`），跑一周看分布再调。
- **交叉验证**：`注册日期 ≤ 证书 first-seen 日期` 才合理；若 first-seen 早于注册日期 → 域名刚易主（老域名新注册），自动降级。
- **失败降级**：RDAP 查不到（如 .ai 部分 TLD）→ 标记 unknown 进 Tier 2，不丢弃。
- 技术：RDAP（非传统 WHOIS），免费无 key，Verisign + 各 TLD RDAP 已验证可用。

### S3 DNS 探测（便宜）
- A 记录存在 = 有解析；MX 存在 = 有邮件（企业建站意图）。
- 无解析 = 囤货/抢注信号（进 Tier 2 或丢弃，待定）。

### S4 L1 HTTP 探测（已有）
- 复用 `DomainHunterPipeline.probe_domain`。

### S5 AI 判型（暂缓）
- 规则预筛（v1 已有 `build_rule_candidate_draft`）+ LLM 判型（MiniMax/oai-compatible，v1 经验）。
- 判型标准待定：是"是不是 AI 工具"还是"是不是值得收录的科技产品"。

## 架构决策（已定）

1. **打分排序 + 阈值**（非硬过滤）：每层产出证据链，review 界面可看"为什么进队列"。硬过滤会漏掉"刚买老域名重新建站"的候选。
2. **RDAP 需要缓存层**：有速率限制，每天几百个查询需要队列 + 缓存 + 重试。这是筛选层唯一需要"工程化"的部分。
3. **阈值全部可配置**：先跑一周真实数据看分布再定稿。
...
- CLI：`domainhunter filter --input domains.json [--tier1-days 30] [--require-dns] [--drop-unknown-rdap]`
