...
_RUNBOOKS: dict[AlertKind, Runbook] = {
    AlertKind.ZERO_INPUT: Runbook(
        ...
        steps=(
            ...
            "确认 CT log 链路是否断流。",
            "能否安全重试："
            "可在 CLI 中手动 domainhunter poll-ct-log --max-probes 1 "
            "验证可达性；"
            "短期补抓不会污染 funnel_metrics，"
            "idempotency_key 保证重放安全。",
            "何时人工介入："
            "持续 1 小时仍无事件需排查上游 CT log 状态页；"
            "若持续静默，按需要下线对应 poller "
    ...
    AlertKind.BASELINE_DRIFT: Runbook(
        ...
        steps=(
            ...
            "无需自动重试——漂移本质是源行为变化；"
            "若判定上游结构变化（如新增 TLD），"
            "domainhunter ingest-ct-page 重放近 1h 增量即可。",
            "何时人工介入："
    ...
    AlertKind.SCHEMA_FAILURE: Runbook(
        ...
        steps=(
            ...
            "配合 domainhunter enrich-llm --provider mock 重放失败样本。",
            "能否安全重试："
            "可对单个 candidate_version 调用 domainhunter enrich-llm "
    ...
    AlertKind.EXTERNAL_SYNC_FAILURE: Runbook(
        ...
        steps=(
            ...
            "可在 CLI 中 domainhunter outreach --request-id … 重发；"
