...
TAXONOMY_VERSION = "domainhunter-taxonomy-v1"
...
def parse_llm_candidate_output(
    ...
    """Accept only fixed-schema output that cites facts already observed by DomainHunter."""
    try:
    ...
    try:
        if not isinstance(payload["is_candidate"], bool):
            raise TypeError("is_candidate must be boolean")
        confidence = payload["classification_confidence"]
        if isinstance(confidence, bool) or not isinstance(confidence, int | float):
            raise TypeError("classification_confidence must be a number")
